...
3. 接入 `camera_display_top.v`:新增两个顶层输出端口 `seg7[6:0]`/`seg7_dp`(有详细注释说明还没配引脚)。新增一段"BNN摄像头流水线":`roi_binarize_28x28` 常驻跑在背景,`img_valid` 每帧触发一次一个小状态机(`BNN_IDLE→BNN_START→BNN_WAIT`)去起 `bnn_core`(跟 `bnn_demo_top.v` 里那个 FSM 是同一个思路,只是触发源从按键换成了 `img_valid`)分类,分类完锁存 `bnn_digit`/置位 `bnn_result_valid`,喂给 `seg7_decoder`。这条路径完全独立于 key1/2/3 那三个按钮选的显示模式 mux——不碰 `final_r/g/b`/`disp_mode`,是真正"常驻后台"的分类器,跟三个按钮已经占用完的模式选择互不冲突(用户在设计讨论阶段就明确这个决定,没有再单独确认,因为三个按钮确实都已经用掉了,没有更省事的接法)。用 iverilog 对整个 `camera_display_top.v`(含新模块)做了一次纯语法/端口连线检查(Analysis-only,没跑仿真波形,因为完整跑一次480x272摄像头光栅+SDRAM+PLL的行为级仿真这个项目里从来没搭过,PLL/SDRAM 用的是 Quartus 专用宏功能 altpll/dcfifo,iverilog 本来就编不出来)——除了这两个已知的、跟这次改动无关的宏功能报"unknown module"之外,新增的 `roi_binarize_28x28`/`bnn_core`/`seg7_decoder` 三处例化全部端口宽度、连线正确,没有报错。
4. qsf:加了 `roi_binarize_28x28.v`/`seg7_decoder.v` 两个 `VERILOG_FILE`。**顺手把 TOP_LEVEL_ENTITY 从遗留的 `bnn_demo_top` 改回了 `camera_display_top`**(bnn_demo_top 真机验证那次跑完之后一直没切回来,这次要继续改 camera_display_top 所以顺手切回)。`seg7`/`seg7_dp` 两个新端口**故意没配 `set_location_assignment`**——问过用户 LG3661BH 的 a~g+小数点具体接哪几个 FPGA 引脚,用户前一轮先确认了极性(共阳极、低电平点亮),但问引脚编号那轮回复的是"LG2661BH"(看着像是在纠正/重复型号本身,不是在给引脚编号,也没法判断是"型号打错了"还是别的意思),没有实际给出引脚信息,所以这轮不猜引脚,先把两个端口露出来、注释里写清楚"占位,等确认引脚后取消注释填入"。不配引脚不影响编译:Quartus 会自动挑2个空闲引脚顶上,能编译通过,只是在这两个引脚真正接到 LG3661BH 之前,显示不会正确。
5. **用户还没编译验证。已知的开放问题,按优先级排:**
   (a) **LG3661BH 具体接哪几个 FPGA 引脚还没确认**——上面第4条的那个悬而未决的"LG2661BH"回复,下次交互需要先问清楚,再回填 qsf 里那一段被注释掉的 `set_location_assignment`。
...
## 踩坑记录(重要)
- **qsf/camera_display_top.v/AGENTS.md 常在写入后被截断**:这个session里反复出现的问题——用 Write/Edit 工具确认写入成功、用 Read 工具读回也显示内容正确,但过一会儿用 bash 读挂载路径下的同一个文件,内容会在某个中间位置突然截断(经常是刚好卡在一条语句中间,或者一个中文 UTF-8 多字节字符中间,有时是尾部被灌入大量空字节而不是截断)。这不是猜测,这个 session 里在 sobel_edge.v / nms_thresh.v / tb_sobel_edge.v / tb_nms_thresh.v / tb_integration_nms.v / camera_display_top.v / CameraCapture.qsf / AGENTS.md / tb_barcode_decoder.v / ean13_decoder.v / bnn_core.v / bnn_demo_top.v / tb_bnn_demo_top.v / roi_binarize_28x28.v(接入camera_display_top.v/qsf/AGENTS.md这一轮再次发生,camera_display_top.v/CameraCapture.qsf/AGENTS.md本身都又中招了一次)上都真实发生过,其中 CameraCapture.qsf 和 AGENTS.md 各自发生了不止一次(不止两次,累计已经四五次以上)。**固定的修复流程**:每次大改动 Write/Edit 后,必须立刻用 bash 读回校验(校验字节数、校验空字节数是否为0、校验能否 utf-8/ascii decode、校验文件尾部 tail 是否是预期的结尾内容,比如 `endmodule`);一旦发现异常,不要再用 Edit 工具小修小补,直接用已知正确的完整内容(通常就是刚才 Write/Edit 工具确认过的那份,或者用 Read 工具重新读一次)整个通过 heredoc 或 Python 脚本(`open(path,'w',encoding='utf-8')`,ASCII-only 的 Verilog 文件也可以直接 heredoc)强制整体重写一遍,重写后再校验一次,必要时要重复2-3轮才能稳定,而且校验本身有时也会撞见同一次瞬时的坏状态,连续多跑几次校验确认稳定下来再往下走
...
LG3661BH(共阳极,低电平点亮,seg7[6:0]={a,b,c,d,e,f,g}):**引脚尚未确定**,qsf里seg7/seg7_dp两个端口没有location assignment,等用户确认具体接线后回填
...
8. BNN子项目已在真机上验证成功(见上面第9条),摄像头→CNN→LG3661BH 整条链路代码已写完接入(见第11条),**用户还没编译验证**。下一步需要:(a)确认LG3661BH的a~g+小数点接哪几个FPGA引脚(用户上一轮回复"LG2661BH"没给出引脚编号,需要再问一次),回填qsf里注释掉的set_location_assignment;(b)编译一次拿真实LE/LAB数字,确认能不能跟主流程共存;(c)镜头对焦问题(用户已明确选择"先不管"直接写代码,但要验证真实识别效果迟早要解决);(d)THRESH/ROI尺寸等参数大概率需要拿真实手写数字实测调参,跟颜色阈值调参脚本(第4条)类似的思路
